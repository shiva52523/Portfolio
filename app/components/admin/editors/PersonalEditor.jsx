...
export default function PersonalEditor() {
  ...
  const [saved, setSaved] = useState(false);
  const [loading, setLoading] = useState(false); // Saving loader track karne ke liye
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      // 1. Safe side temporary local state cache me backup rakhna
      localStorage.setItem('admin_personal', JSON.stringify(data));

      // 2. Next.js API hit karke physical file System file overwrite karna
      ...
      console.error(err);
      alert('❌ Server connection failure! Backend API reach nahi ho paayi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = () => {
    if (confirm("Kya aap such me saari personal details ko reset karna chahte hain?")) {
      setData({ ...defaultData });
      localStorage.removeItem('admin_personal');
    }
  };
  ...
  return (
    <div>
      {saved && <SuccessToast message="Personal data saved permanently to local files!" />}
      <InfoBanner />

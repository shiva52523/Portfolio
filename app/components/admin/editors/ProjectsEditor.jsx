...
export default function ProjectsEditor() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('❌ Server failure. API hit nahi ho paayi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = () => {
    if (confirm("Kya aap sach me saare Projects data ko default par reset karna chahte hain?")) {
      setItems([...defaultData]);
      localStorage.removeItem('admin_projects');
    }
  };
  ...
  return (
    <div>
      {saved && <SuccessToast message="Projects saved permanently to local files!" />}
      <InfoBanner />

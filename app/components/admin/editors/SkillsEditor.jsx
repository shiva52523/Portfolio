...
import { useState } from 'react';
import { skillsData as defaultData } from '@/utils/data/skills';
import { cardStyle, sectionTitle, btnPrimary, SuccessToast, InfoBanner } from './shared';
...
export default function SkillsEditor() {
  ...
  // Naya Backend API integrated handleSave function
  const handleSave = async () => {
    setLoading(true);
    try {
      // 1. Safe side temporary local state cache me backup
      localStorage.setItem('admin_skills', JSON.stringify(skills));

      // 2. Next.js API hit karke physical file overwrite karna
      ...
      console.error(err);
      alert('❌ Server connectivity fail. API route path invalid hai ya crash hua hai.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = () => {
    if (confirm("Kya aap sach me saare Skills data ko default par reset karna chahte hain?")) {
      setSkills([...defaultData]);
      localStorage.removeItem('admin_skills');
    }
  };
  ...
  return (
    <div>
      {saved && <SuccessToast message="Skills saved permanently to local files!" />}
      <InfoBanner />
      ...
      <div style={{ ...cardStyle }}>
        ...
        {skills.length === 0 && (
          <div style={{ textProject: 'center', color: '#64748b', padding: '40px', fontSize: '14px' }}>
            No skills added yet. Add some using the form above.
          </div>
        )}
      </div>
